...
class JHipsteerTestAppKafkaResourceIT {
    ...
    void setup() {
        ...
        JHipsteerTestAppKafkaResource kafkaResource = new JHipsteerTestAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/j-hipsteer-test-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/j-hipsteer-test-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
